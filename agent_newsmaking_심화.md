...
## 작업 파이프라인 (요청하면 이 순서대로 실행한다)
"파이프라인 실행" 같은 지시를 받으면 0단계부터 순서대로 진행하고, 각 단계가 끝날 때마다 무엇을 했는지 한 줄로 보고한다. 모호하거나 위험하면 멈추고 사람에게 묻는다. "N단계만", "3단계까지만" 같은 부분 실행도 지원한다.

- 0. PDF 변환: pdf-to-md 스킬로 sources/의 PDF를 sources_md/로 변환. 빈 결과(스캔본)는 표시. → sources_md/*.md
- 1. 문체 학습(선택): style/my-articles/를 분석해 문체 규칙 정리. → output/style_guide.md
- 2. 핵심 추출: extractor 서브에이전트 + extract-key-points 스킬로 사실 대장 작성, 표 수치는 원본 PDF로 교차 확인. → output/fact_ledger.md
- 3. 검색 보완: researcher 서브에이전트로 '검색 필요' 항목·배경을 웹에서 보완(URL·날짜 표기). → output/research_notes.md
- 4. 기사 작성: write-article 스킬로 사실 대장·리서치 노트만 근거로, 뉴스가치로 앵글을 정해 내 문체로 작성(문장에 근거 id). → output/drafts/article_v1.md
- 5. 팩트체크: fact-checker 서브에이전트로 사실·수치·인용을 사실 대장·원문에 대조 검증. → output/factcheck_report.md
- 6. 수정 반영: 팩트체크 지적을 반영하고 변경 요약을 붙인다. 남은 [확인 필요]는 사람 확인 목록으로. → output/drafts/article_v2.md

각 단계 결과는 반드시 output/에 파일로 남긴다. 근거 없는 문장은 쓰지 않는다.
...
작업 순서(0~6단계)는 이미 CLAUDE.md의 "작업 파이프라인"에 정의해 두었습니다. 그래서 매번 단계별 프롬프트를 길게 입력할 필요 없이, 실행만 지시하면 Claude Code가 정해진 순서대로 서브에이전트와 스킬을 불러 진행합니다.

### 전체 실행 (기본)

```text
CLAUDE.md의 작업 파이프라인을 0단계부터 끝까지 실행해줘.
각 단계 산출물을 output/에 저장하고, 단계마다 무엇을 했는지 한 줄로 보고해.
모호하거나 위험하면 멈추고 물어봐.
```

### 부분·단계별 실행 (실습용)

기능을 하나씩 체감하고 싶을 때는 단계를 끊어 지시하면 됩니다. 어떤 서브에이전트·스킬이 언제 작동하는지 눈으로 확인하기 좋습니다.

```text
파이프라인 2단계(핵심 추출)만 실행해줘.
```

```text
3단계까지만 실행하고 멈춰줘. 사실 대장과 리서치 노트를 확인할게.
```

### 더 간단하게 — 커스텀 명령으로 만들기 (선택)

아래 파일을 만들어 두면, 매번 문장을 입력하지 않고 `/run` 한 번으로 파이프라인을 실행할 수 있습니다.

.claude/commands/run.md

```markdown
CLAUDE.md의 "작업 파이프라인"을 0단계부터 끝까지 실행한다.
각 단계 산출물을 output/에 저장하고, 단계마다 한 줄로 보고한다.
인자가 있으면 그 단계만 실행한다: $ARGUMENTS
```

이제 입력창에 `/run`(전체) 또는 `/run 2단계만`처럼 쓰면 됩니다.

### 권장 운영

처음에는 Shift+Tab으로 플랜 모드에 들어가 전체 계획을 먼저 본 뒤 실행하세요. 어떤 단계에서 어떤 서브에이전트·스킬이 작동하는지 한눈에 보여, 교육용으로 특히 좋습니다.
...
- [ ] CLAUDE.md 작업 파이프라인을 실행했는가 (전체 또는 단계별)
